    ...
    <>
      ...
      <div className="flex flex-1 flex-col gap-4 p-6">
        <div className="flex flex-1 flex-col">
          ...
          <p className="text-body line-clamp-4 text-base leading-normal">
            {item.description}
          </p>
        </div>

        {item.href && item.linkLabel ? (
          <span className="text-link-cta mt-auto inline-flex items-center gap-2 text-sm font-semibold leading-[21px] transition-opacity group-hover:opacity-80">
            {item.linkLabel}
            <ArrowIcon />
          </span>
        ) : null}
...
export function SectorsGridCard({ item, className }: SectorsGridCardProps) {
  const articleClassName =
    'flex h-full flex-col overflow-hidden rounded-lg border border-black/10 bg-white transition-shadow group-hover:shadow-md';
  ...
  return (
    <article
      className={cn(
        'flex h-full flex-col overflow-hidden rounded-lg border border-black/10 bg-white',
        className,
      )}
    >
      <SectorsGridCardContent item={item} />
